...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      const value = JSON.stringify({ command: p.command.slice(0, 2000), exitCode: p.exit_code, stdout: (p.stdout || "").slice(-8000), stderr: (p.stderr || "").slice(-8000), durationMs: p.duration_ms });

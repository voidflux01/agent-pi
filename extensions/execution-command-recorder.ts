...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_execution_end", async (event: any) => {
    ...
    recordEvidence(start.runDir, { id: `runtime-${Date.now().toString(36)}`, type: event.toolName === "run_tests" ? "test" : "command", source: "runtime", value: JSON.stringify({ command: start.command, exitCode: details.exitCode ?? (event.isError ? 1 : 0), output: text.slice(-MAX_OUTPUT), durationMs: Date.now() - start.startedAt }), timestamp: new Date().toISOString() });
  });
}

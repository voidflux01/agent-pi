...
describe("orchestration query", () => {
	test("summarizes persisted run events", () => {
		...
		expect(summary).toMatchObject({ runId: run.runId, parentRunId: "parent-1", actor: "test", mode: "PLAN", status: "succeeded", eventCount: 6, totalTokens: 10, costUsd: 0.01, verificationStatus: "PASS", verificationPassed: 2, verificationFailed: 0, changedFiles: ["src/a.ts"] });

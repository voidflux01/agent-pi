...
import { cleanupRuntimeArtifacts, cleanupSessionScrap, cleanupVerifierTranscripts, LAST_SWEEP_MARKER } from "../lib/runtime-cleanup.ts";
...
describe("retention sweep", () => {
	it("removes old files, keeps fresh files, and reaps emptied dirs", () => {
		write(".pi/agent-sessions/dispatch-receipts/old.jsonl", OLD);
		const fresh = write(".pi/agent-sessions/dispatch-receipts/fresh.jsonl");
		write(".context/evidence/old.jsonl", OLD);
		const kept = write(".context/evidence/new.jsonl");

		cleanupRuntimeArtifacts(cwd);

		expect(existsSync(join(cwd, ".pi/agent-sessions/dispatch-receipts/old.jsonl"))).toBe(false);
...
describe("throttle", () => {
	it("skips the sweep while the marker is fresh, runs after it ages out", () => {
		...
		write(".pi/agent-sessions/dispatch-receipts/run-a.jsonl", OLD);
		cleanupRuntimeArtifacts(cwd);
		expect(existsSync(join(cwd, ".pi/agent-sessions/dispatch-receipts/run-a.jsonl"))).toBe(false);

		// A second old file appears; the fresh marker suppresses the next sweep.
		write(".pi/agent-sessions/dispatch-receipts/run-b.jsonl", OLD);
		cleanupRuntimeArtifacts(cwd);
		expect(existsSync(join(cwd, ".pi/agent-sessions/dispatch-receipts/run-b.jsonl"))).toBe(true);
		...
		cleanupRuntimeArtifacts(cwd);
		expect(existsSync(join(cwd, ".pi/agent-sessions/dispatch-receipts/run-b.jsonl"))).toBe(false);
	});
});

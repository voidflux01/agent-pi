...
import { cleanupRuntimeArtifacts, cleanupSessionScrap, cleanupVerifierTranscripts } from "./lib/runtime-cleanup.ts";

export default function (pi: ExtensionAPI) {
	pi.on("session_start", async (_event, ctx) => {
		// Worker processes never clean: a subagent's artifacts may still be needed
		// by the parent (resume/recovery), so cleanup only happens at the parent

...
export default function(pi: ExtensionAPI) {
 ...
 async function runChain(
  ...
  const autonomous = await autonomousFinalize({ cwd: ctx.cwd || process.cwd(), mode: "CHAIN", taskText: task, risk: "low", dispatchRepair: builderRepairDispatcher(ctx) });
  if (autonomous && !autonomous.allowed) {
   orchestrationRun.finish("failed", { steps: activeChain.steps.length, reason: autonomous.reason });
   return { output: `${input}\nCompletion blocked: ${autonomous.reason || autonomous.status}`, fullOutput: lastFullOutput, fullOutputPath: lastFullOutputPath, success: false, elapsed: Date.now() - chainStart, runId: orchestrationRun.runId };
  }
